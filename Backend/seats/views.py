...
class create_layout(APIView):
    ...
    def post(self, request):
        ...
        if SeatScreenLayout.objects.filter(name__icontains=data["name"]).exists():
            return Response(
                {"error": f"layout {name} is already exists"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if int(row) <= 2 or int(col) <= 2:
            return Response(
                {"error": "enter a valid rows and seats format"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class get_screen_seats(APIView):
    ...
    def get(self, request, screen_id):
        try:
            ...
            seatss = seats.objects.filter(screen=screen_id, is_active=True).order_by(
                "row", "number"
            )
                ...
                seat_data.append(
                    {
                        ...
                        "is_booked": BookingSeat.objects.filter(
                            seat=seat.id, booking__show_id=show_id, status="booked"
                        ).exists(),
...
class get_theatre_screens(APIView):
    ...
    def get(self, request, id):
        try:
            ...
            return Response({"screen_data": data}, status=status.HTTP_200_OK)
...
class update_seats_category(APIView):
    ...
    def post(self, request):
        ...
        if not seats.objects.filter(id__in=seats_ids).exists():
            return Response(
                {"error": "no seats are selected"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class Lock_seats(APIView):
    ...
    def post(self, request):
        ...
        if locked_seats.exists():
            return Response(
                {"error": "some seats are not available‼️"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def post(self, request):
        try:
            ...
            try:
                ...
                return Response(
                    {"eror": "show does not found"}, status=status.HTTP_400_BAD_REQUEST
                )

            seats = SeatLock.objects.filter(
                seat_id__in=seat_ids, show_id=show_id, expires_at__gt=now
            )

            if action == "eventbacked" or seats.exists():
                seats.delete()
                return Response(
                    {"message": "seats has been unloked with user event"},
                    status=status.HTTP_200_OK,
                )
