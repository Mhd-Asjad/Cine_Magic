...
class CityTheatreView(APIView):
    ...
    def get(self, request, city_id):
        try:
            ...
            response_data = {"cityname": city.name, "theatres": serialzer.data}
            return Response(response_data, status=status.HTTP_200_OK)
        except City.DoesNotExist:
            return Response(
                {"detail": "no theatre found"}, status=status.HTTP_404_NOT_FOUND
            )
...
class AddTheatre(APIView):
    ...
    def post(self, request, owner_id):
        ...
        try:
            ...
            theatre = Theatre.objects.create(name=name, city=city, address=address)
            return Response(
    ...
    def get(self, reqeust):
        ...
        for movie in queryset:
            ...
            movie_dict = {
                ...
                "genre": movie.genre,
                "poster": (
                    reqeust.build_absolute_uri(movie.poster.url)
                    if movie.poster
                    else None
                ),
            }
    ...
    def post(self, request):
        try:
            ...
            if action == "confirmed":
                ...
                screen_data = Screen.objects.filter(
                    theatre=theatre_det, is_approved=False
                )
                if screen_data:
                ...
                send_response_mail.delay(theatre_det.owner.user.email, subject, msg)
                return Response(
...
def Verified_Theatres(request):
    try:
        theatres = Theatre.objects.all().order_by("-created_at")
    except Theatre.DoesNotExist:
        return Response(
            {"error": "no theatres found"}, status=status.HTTP_404_NOT_FOUND
        )
...
class verify_screen(APIView):
    ...
    def post(self, request, screen_id):
        ...
        try:
            ...
            email_msg = (
                f"screen updated by cineMagic admins \n\n wish you a good start🎉"
            )

            send_response_mail.delay(owner.user.email, subject, email_msg)
            return Response(
                {"message": "approved successfully"}, status=status.HTTP_200_OK
            )

        except Screen.DoesNotExist:
            return Response(
                {"error": "not found screen"}, status=status.HTTP_404_NOT_FOUND
            )

    def delete(self, request, screen_id):
        logger.info(f"DELETE payload: {request.data}")
        try:
            data = request.data
        ...
        screen.rejection_reason = rejection_reason
        screen.save()
        try:
            owner = TheaterOwnerProfile.objects.get(id=owner_id)
        except TheaterOwnerProfile.DoesNotExist:
            logger.error(f"TheaterOwnerProfile with id {owner_id} not found")
            return Response({"error": "Owner not found"}, status=404)
        logger.info(
            f"owner email spotted : {owner.user.email} and rejection reason : {rejection_reason}"
        )
        subject = f"Screen : {screen_number} from {theatre_name} Rejected By Admins "
        message = f"""
            Dear {owner.user.username},

            We’ve reviewed your request to activate Screen {screen_number} at {theatre_name}.
        ...
        """
        send_response_mail.delay(owner.user.email, subject, message)

        return Response(
            {
                "message": f"Screen : {screen_number} from {theatre_name} deleted successfully  "
            },
            status=status.HTTP_200_OK,
        )


# to handle theatre screen status toggle
@api_view(["PATCH"])
def toggle_screen_status(request, pk):
    try:
        ...
        return Response(
            {
                "message": f'Status updated to {"Active" if screen.is_active else "Inactive"}'
            }
        )
    except Screen.DoesNotExist:
...
# to handle theatre status toggle
@api_view(["PATCH"])
def toggle_theatre_status(request, pk):
    try:
        theatre = Theatre.objects.get(pk=pk)
        theatre.is_confirmed = not theatre.is_confirmed
        theatre.save()
        return Response(
            {
                "message": f'Theater status Update to {"Active" if theatre.is_confirmed else "Inactive"}'
            }
        )
...
def Cancel_Show(request, show_id):
    ...
    show.delete()
    return Response(
        {"message": f"{movie} on screen {screen_number} was successfully cancelled"}
    )
...
class PendingCancelledShows(APIView):
    def get(self, request):
        try:
            ...
            return Response(
                {"error": "not booking found"}, status=status.HTTP_404_NOT_FOUND
            )
...
class dashboard_stats(APIView):
    ...
    def get(self, request):
        ...
        booking_queryset = Booking.objects.filter(
            booking_time__range=[start_date, end_date], status="confirmed"
        )
        ...
        active_users = (
            User.objects.filter(booking__in=booking_queryset).distinct().count()
        )

        active_theatres = (
            Theatre.objects.filter(
                screens__showtimes__booking__in=booking_queryset, is_confirmed=True
            )
            .distinct()
            .count()
        )
        ...
        previous_booking_queryset = Booking.objects.filter(
            booking_time__range=[previous_stats, start_date], status="confirmed"
        )

        previous_revenue = (
            previous_booking_queryset.aggregate(Sum("amount"))["amount__sum"] or 0
        )
        ...
        ticket_change = (
            (
                (total_tickets - previous_booking_queryset.count())
                / previous_booking_queryset.count()
            )
            * 100
...
class revenue_chart_data(APIView):
    ...
    def get(self, request):
        ...
        logger.info(
            f"Received period: {period}, start_date: {start_date}, end_date: {end_date}"
        )
        ...
        revenue_data = (
            ...
            .annotate(period=trunc_func("booking_time"))
            .values("period")
            .annotate(
                total_revenue=Sum("amount"),
                admin_revenue=ExpressionWrapper(
                    Sum("amount") * 0.10, output_field=FloatField()
                ),
                theatre_revenue=ExpressionWrapper(
                    Sum("amount") * 0.90, output_field=FloatField()
                ),
...
class ticket_trend_data(APIView):
    ...
    def get(self, request):
        ...
        ticket_data = (
            ...
            .annotate(month=TruncMonth("booking__booking_time"))
            .values("month")
        ...
        ticket_dict = {
            item["month"].strftime("%b"): item["tickets"] for item in ticket_data
        }
        ...
        while month_pointer <= end_date:
            ...
            result.append(
                {"month": month_abbr, "tickets": ticket_dict.get(month_abbr, 0)}
            )
            if month_pointer.month == 12:
                month_pointer = month_pointer.replace(
                    year=month_pointer.year + 1, month=1
                )
            else:
    ...
    def get(self, request):
        ...
        try:
            recent_bookings = (
                ...
                .select_related("user", "show__movie", "show__screen__theatre")
                ...
                sales_data.append(
                    {
                        ...
                        "theatre": (
                            ...
                            if booking.show and booking.show.screen
                            else "N/A"
                        ),
                        "time": time_ago,
                        "tickets_count": booking.bookingseats.filter(
                            status="booked"
                        ).count(),
                    }
                )
...
class ExportTheatreReport(APIView):
    ...
    def get(self, request):
        ...
        if report_type == "all" or report_type == "detailed_performance":
            create_detailed_performance_sheet(
                wb, bookings_queryset, start_date, end_date
            )
...
def create_theatre_summary_sheet(wb, bookings_query, start_date, end_date):
    ...
    for theatre in theatres:
        theatre_bookings = bookings_query.filter(show__screen__theatre=theatre)

        total_revenue = theatre_bookings.aggregate(Sum("amount"))["amount__sum"] or 0
        total_tickets = BookingSeat.objects.filter(
            booking__in=theatre_bookings, status="booked"
        ).count()
        ...
        theatre_data.append(
            {
                ...
                "Revenue per Show": (
                    float(total_revenue / total_shows) if total_shows > 0 else 0
                ),
            }
        )
...
def create_seat_category_sheet(wb, bookings_query, start_date, end_date):
    ...
    for theatre in theatres:
        ...
        seat_categories = (
            BookingSeat.objects.filter(booking__in=theatre_bookings, status="booked")
            .annotate(category_name=Coalesce(F("seat__category__name"), V("Standard")))
            ...
            seat_data.append(
                {
                    ...
                    "Average Price": (
                        float(category["revenue"] / category["count"])
                        if category["count"] > 0
                        else 0
                    ),
                }
    ...
    if not df.empty:
        category_summary = (
            df.groupby("Seat Category")
            .agg({"Tickets Sold": "sum", "Revenue": "sum"})
            .reset_index()
        )
        category_summary["Average Price"] = (
            category_summary["Revenue"] / category_summary["Tickets Sold"]
        )
            ...
            detailed_data.append(
                {
                    ...
                    "Show Date": (
                        booking.show.show_date.strftime("%Y-%m-%d")
                        if booking.show.show_date
                        else "N/A"
                    ),
                    "Amount": float(booking.amount),
            ...
            monthly_data.append(
                {
                    ...
                    "Avg Booking Value": (
                        float(stat["revenue"] / stat["bookings_count"])
                        if stat["bookings_count"] > 0
                        else 0
                    ),
                }
            )
...
def style_worksheet(ws, data_rows):
    ...
    for idx, row in enumerate(ws.iter_rows(), 1):
        if any(
            ...
            and cell.value in ["Theatre Name", "Theatre", "Booking ID"]
        ):
            header_row = idx
            break
...
def GetTheatres(request):
    ...
    for booking in bookings:
        ...
        if theatre.id not in seen_theatre_id:
            ...
            data.append(
                {"id": theatre.id, "name": theatre.name, "city": theatre.city.name}
            )
